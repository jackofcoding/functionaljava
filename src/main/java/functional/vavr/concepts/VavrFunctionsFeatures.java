...
public class VavrFunctionsFeatures {
   ...
   */
  public <T, U, R> List<R> compose(
      List<T> items, Function1<T, U> firstOps, Function1<U, R> secondOps) {
    ...
    return items.stream().map(e -> secondOps.compose(firstOps).apply(e)).toList();
  }
